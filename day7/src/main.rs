...
impl<'a> Amplifiers<'a> {
    fn new(init_mem: &'a Vec<i32>, phase_settings: &'a [usize], config: Configuration) -> Self {
        Self {
            init_mem,
            phase_settings,
            config,
...
fn maximize_amplifiers(init_mem: &Vec<i32>, config: Configuration) -> isize {
    fn max_amp_util(
        init_mem: &Vec<i32>,
        config: Configuration,
