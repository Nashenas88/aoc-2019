...
impl<T> Io for Box<T> where T: Io {
    fn read(&self) -> String {
        (**self).read()
    }
...
where
    T: Io + 'a,
{
    pub fn new(mem: &'a mut Vec<i128>, io: T) -> Self {
        Program {
            mem,
            ctr: 0,
            rel: 0,
...
impl PipedIo {
    pub fn new(rx: (String, Receiver<String>), tx: (String, Sender<String>)) -> Self {
        Self { rx, tx }
    }

    pub fn init_input<S: Into<String>>(&mut self, input: S) {
        self.tx
            .1
            .send(input.into())
...
pub fn parse(file_name: &str) -> Result<Vec<i128>, String> {
